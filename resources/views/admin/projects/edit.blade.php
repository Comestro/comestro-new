...
<div class="bg-white rounded-lg shadow-sm p-6">
    ...
    <form action="{{ url('/admin/projects/' . $project->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $project->slug) }}" required
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    <p class="text-xs text-gray-500 mt-1">URL-friendly name for your project.</p>
                    @error('slug')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="Enterprise Solution" {{ old('category', $project->category) == 'Enterprise Solution' ? 'selected' : '' }}>Enterprise Solution</option>
                    </select>
                    @error('category')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="completion_date" id="completion_date" 
                        value="{{ old('completion_date', $project->completion_date ? date('Y-m-d', strtotime($project->completion_date)) : '') }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" required
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('description', $project->description) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" required
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content', $project->content) }}</textarea>
        ...
        <div class="bg-gray-50 p-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Leave empty to keep current image. Recommended size: 1200x800px.</p>
                    @error('image')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="current_image" class="block text-sm font-medium text-gray-700 mb-1">Current Image</label>
                    <div id="image_preview" class="h-32 bg-gray-100 border rounded-md flex items-center justify-center text-gray-400">
                        @if($project->image)
                            <img src="{{ asset('storage/' . $project->image) }}" alt="{{ $project->title }}" class="h-full object-contain">
                        @else
        ...
        <div class="bg-gray-50 p-4 rounded-md">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="url" name="website_url" id="website_url" value="{{ old('website_url', $project->website_url) }}"
                        placeholder="https://"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    @error('website_url')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="url" name="case_study_url" id="case_study_url" value="{{ old('case_study_url', $project->case_study_url) }}"
                        placeholder="https://"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
