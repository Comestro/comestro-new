                    ...
                    @forelse($messages as $msg)
                        <tr class="hover:bg-gray-50 transition duration-200">
                            ...
                            <td class="px-6 py-4 text-center flex justify-center space-x-3">
                                <a href="{{route('admin.viewmsg', $msg->id)}}" class="text-indigo-600 hover:text-indigo-800 flex items-center space-x-1">
                                    <i class="fas fa-eye"></i>
                                    <span>View</span>
                                </a>
                                <form action="{{ route('admin.deleteContact', $msg->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this message?');">
                                    @csrf
                                    @method('DELETE')
